...
const MyGoals: NextPageWithLayout = () => {
  ...
  const activeGoals = getGoalsByAddress(getGoalsByStatus(allGoals, 0), address);
  const achievedGoals = getGoalsByAddress(getGoalsByStatus(allGoals, 1), address);
  const abandonedGoals = getGoalsByAddress(getGoalsByStatus(allGoals, 2), address);
  return (
    <>
      <section className="mx-auto w-full max-w-[1160px] text-sm sm:pt-10 4xl:pt-14">
        ...
        >
          <TabPanel className="focus:outline-none">
            <GoalList goals={getGoalsByAddress(getGoalsByStatus(allGoals, 0), address)} />
          </TabPanel>
          <TabPanel className="focus:outline-none">
            <GoalList goals={getGoalsByAddress(getGoalsByStatus(allGoals, 1), address)} />
          </TabPanel>
          <TabPanel className="focus:outline-none">
            <GoalList goals={getGoalsByAddress(getGoalsByStatus(allGoals, 2), address)} />
          </TabPanel>
        </ParamTab>
      </section>
    </>
...
MyGoals.getLayout = function getLayout(page) {
